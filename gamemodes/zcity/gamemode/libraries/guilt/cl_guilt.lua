...
hook.Add("Player Spawn", "GuiltKnown",function(ply)
    ...
    --    ply:ChatPrint("Текущая карма "..tostring(math.Round(ply.Karma)).."")
    --end
end)
...
local function harmdone(harm)
    if harm >= 9 then
        return "погромил тебя."
    elseif harm >= 5 then
        return "погромил тебя."
    elseif harm >= 2 then
        return "серьёзно тебя ранил."
    elseif harm >= 1 then
        return "чуточку тебя ранил."
    else
        return "легонько тебя шлёпнул."
    end
end

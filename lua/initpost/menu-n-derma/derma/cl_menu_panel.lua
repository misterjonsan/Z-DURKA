...
DISCORD_URL = "https://discord.gg/jneYerVQCh"
...
local Selects = {
    ...
    {Title = "Дискорд сервер", Func = function(luaMenu) luaMenu:Close() gui.OpenURL(DISCORD_URL)  end},
    {Title = "Роли трейтора",
    GamemodeOnly = true,
    ...
    {Title = "Скин", Func = function(luaMenu,pp) hg.CreateApperanceMenu(pp) end},
    {Title = "Обратно играть", Func = function(luaMenu) luaMenu:Close() end},
}

local splasheh = {
    'FREAK CITY говно',
    'зайдите в наш дискорд',
    'фрекс вайбкодер',
    'что такое хомиград',
    '!rtv',
    'админ абьюз',
    'нетка бекдур умные вещи програмирование - фрекс',
    'JOHN Z-CITY',
    'плывка не смешно',
    'что такое садсалат',
    'rainworld не стоит того',
    'больше вайбкодинга',
    'измени роль трейтора гнида',
    'качез и пижма кенты',
    'елисей реален?'
}

--print(string.upper('качез и пижма кенты'))
...
function PANEL:InitializeMarkup()
    ...
    if hg.PluvTown.Active then
        local text = "<font=ZC_MM_Title><colour=199,2,2>    </colour>Durka</font>\n<font=ZCity_Tiny><colour=105,105,105>" .. gm .. "</colour></font>"
    ...
    local text = "<font=ZC_MM_Title><colour=199,2,2,255>Z</colour>-Durka</font>\n<font=ZCity_Tiny><colour=105,105,105>" .. gm .. "</colour></font>"
    return markup.Parse(text)
end
...
function PANEL:Init()
    ...
    zteam:SetText("ZDurka Creators: kachezzz, evstapg, lexa_koder")

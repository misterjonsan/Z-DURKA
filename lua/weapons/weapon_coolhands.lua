...
SWEP.Spawnable = true
SWEP.AdminOnly = true -- Changed to admin only
...
SWEP.BreakBoneMul = 100000
SWEP.Penetration = 100000 -- Increased penetration massively
SWEP.DamageMul = 500000 -- Increased damage multiplier massively
SWEP.animtime = 0
...
SWEP.DamagePrimary = 1000 -- Increased base damage massively
...
function SWEP:AttackFront(special_attack, rand)
	...
	if IsValid(Ent) or (Ent and Ent.IsWorld and Ent:IsWorld()) then
		...
		-- Significantly increased damage for admin fists
		local DamageAmt = (math.random(500, 1000) * (special_attack and 3 or 1)) * (self.DamageMul or 1)
		local ent = Ent
		local vec = AimVec
		...
		if string.find(ent:GetClass(),"prop_") and not ent:IsRagdoll() then
			ent:CallOnRemove("gibbreak",function()
				ent:GibBreakClient( vec * 1000 ) -- Increased force for gibbing
			end)
		...
		Mul = Mul * (owner.MeleeDamageMul or 1) * 50 -- Massive multiplier for admin fists
		...
		if Ent:IsPlayer() then
			Ent:ViewPunch(Angle(special_attack and -90 or -30,0,0)) -- Increased viewpunch for victims
		end
		...
		if IsValid(Phys) then
			...
			Phys:ApplyForceOffset(AimVec * 50000 * Mul * (isZomb and 3 or 1), HitPos) -- Increased force massively
